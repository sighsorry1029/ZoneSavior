...
Every `zones.yml` rule must include an explicit non-negative `limit`. ZoneSavior parses the current schema directly and does not use a file-level version field.
